---
title: '02 - PubliliaFitness - Defining strategies and calculating frequencies'
...
All_data_v6 and strats_data should be all we need
```{r}
...
## Not visited early
# ever ants b4h
ggplot(data = filter(strats_data, 
...
```{r}
Fig1_v1_data <- strats_data %>%
  filter(!grepl("U", Female_ID), !females_removed, mom_at_init,
         clutch_initiated_order == 1, visit_ends < last_day_fID)

strat_data_table1 <- strats_data %>%
...
Fig1_v1C.gd <- ggplot(data = filter(strats_data,
...
filter(strats_data, `visited in 4 days` == "Not visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1, visit_ends < last_day_fID,
                                 initL) %>% nrow()
...
filter(strats_data, `visited in 4 days` == "Visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1, visit_ends < last_day_fID,
                                 initL) %>% nrow()
...
strats_data %>% filter(initL, mom_at_init, num_at_init>1) %>% nrow() # 34
strats_data %>% filter(initS, mom_at_init, num_at_init>1) %>% nrow() # 36

strats_data %>% filter(initL, mom_at_init, num_at_init>1,
                                  !grepl("U", Female_ID), !females_removed) %>% nrow() # 13

strats_data %>% filter(initS, mom_at_init, num_at_init>1,
...
### No tie-breaking.
```{r}
Fig1_v2A <- ggplot(data = filter(strats_data, `visited in 4 days` == "Not visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
            alpha = 0.75, stat='bin', binwidth = 4, size = 0, position = "identity") + 
...
Fig1_v2B <- ggplot(data = filter(strats_data, `visited in 4 days` == "Visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
            alpha = 0.75, stat='bin', binwidth = 4, size = 0, position = "identity") + 
...
```{r}
Fig1_v3A <- ggplot(data = filter(strats_data, `visited in 4 days` == "Not visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1,
                                 initL)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
...
Fig1_v3B <- ggplot(data = filter(strats_data, `visited in 4 days` == "Visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1,
                                 initL)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
...
```{r}
Fig1_v4A <- ggplot(data = filter(strats_data, `visited in 4 days` == "Not visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1,
                                 initS)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
...
Fig1_v4B <- ggplot(data = filter(strats_data, `visited in 4 days` == "Visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1,
                                 initS)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
...
visit_cuts_data_ExDis <- strats_data %>%
...
### no tb
visitors_ExDis_notb <- strats_data %>%
...
filter(strats_data, location == "A18_8", mom_at_init) %>%
...
## Adding secondary visitors
### no tb
visitors_InDis_notb <- strats_data %>%
...
### initL
visitors_InDis_initL <- strats_data %>%
  semi_join(filter(visit_cuts_data, initL), by=c("location")) %>%
  filter(!females_removed, !mom_at_init | !initL, visit_starts >= initiation_date)
...
### initS
visitors_InDis_initS <- strats_data %>%
...
```{r}
Fig1_v5A <- ggplot(data = filter(strats_data, `visited in 4 days` == "Not visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1, visit_ends < last_day_fID,
                                 initL)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
...
Fig1_v5B <- ggplot(data = filter(strats_data, `visited in 4 days` == "Visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order == 1, visit_ends < last_day_fID,
                                 initL)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
...
visitors_ExDis_initL <- strats_data %>%
...
# strats_data %>% filter(location=='C41_6')
# # C41_6 was deserted immediately, then visited long after hatch
# strats_data %>% filter(location=='E148_6')
# # E148_6 was also visited long after hatch
# strats_data %>% filter(location=='E40_9')
...
```{r}
visit_data.desL.desP <- strats_data %>%
...
```{r}
Fig1sup_v2A <- ggplot(data = filter(strats_data, `visited in 4 days` == "Not visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order > 1)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
            alpha = 0.75, stat='bin', binwidth = 4, size = 0, position = "identity") + 
...
Fig1sup_v2B <- ggplot(data = filter(strats_data, `visited in 4 days` == "Visited",
                                 !grepl("U", Female_ID), !females_removed, mom_at_init,
                                 clutch_initiated_order > 1)) +
  geom_area(aes(x = est_rel2h, y=..density.., color = ants, fill = ants), 
            alpha = 0.75, stat='bin', binwidth = 4, size = 0, position = "identity") + 
...
```{r}
ggplot(data = 
         filter(strats_data, !grepl("U", Female_ID), !females_removed, 
                mom_at_init,
                clutch_initiated_order == 1, 
                visit_ends < last_day_fID)) +
...
base_text <- expand.grid(ants = c("Ants removed", "Control"),
            `visited in 4 days` = c("Not visited within first 4 days", 
                                    "Visited within first 4 days")) %>%
  tibble()

init_text <- base_text %>%
  mutate(label = ifelse(ants=="Ants removed" & `visited in 4 days`=="Not visited within first 4 days", 
         "Mean init. date", 
         ""))
...
strats_data %>%
  ...
  geom_text(data = init_text,
            aes(x = -20, y = 10, label = label),
            hjust = 0, vjust = -1, color = "gray", fontface = "bold") +
...
# 2023.09.31 Figure S4

## Original
```{r}

Fig1_v1A.gd <- Fig1_v1_data %>%
  filter(`visited in 4 days` == "Not visited") %>%
  ggplot(data = .) +
  geom_area(aes(x = guard_duration, y=..density.., color = ants, fill = ants), 
            alpha = 0.75, stat='bin', binwidth = 4, size = 0, position = "identity") + 
  theme_publilia() +
...
hist_strips <- strip_nested(
     ...
     background_y = list(
       element_rect(fill = visit_colors[1]), 
       element_rect(fill = visit_colors[2]),
       element_rect(fill = "cornflowerblue"),
       element_blank(), element_blank(), 
       element_blank(), element_blank(),
       element_blank(), element_blank()),
     text_y = list(
...
base_text <- expand.grid(ants = c("Ants removed", "Control"),
            cat4facet = c("Not visited", "Visited", "Visitor")) %>%
  tibble()

hatch_text <- base_text %>%
  mutate(label = ifelse(ants=="Ants removed" & cat4facet=="Not visited", 
         "Hatch date", 
         ""))

Figure_S4_data <- strats_data %>%
  mutate(is_not_visited = !grepl("U", Female_ID) &
           !females_removed &
           mom_at_init & 
           clutch_initiated_order == 1 &
           visit_ends < last_day_fID &
           `visited in 4 days` == "Not visited",
         is_visited = !grepl("U", Female_ID) &
           !females_removed &
           mom_at_init & 
           clutch_initiated_order == 1 &
           visit_ends < last_day_fID &
           `visited in 4 days` == "Visited",
         is_visitor = !females_removed &
           !mom_at_init &
           visit_ends >= initiation_date &
           visit_ends < last_day_fID,
         cat4facet = ifelse(is_visited, "Visited",
                            ifelse(is_not_visited, "Not visited",
                                   ifelse(is_visitor, "Visitor", NA))),
         cat4facet = factor(cat4facet, levels = c("Not visited", "Visited", "Visitor"))) %>%
  filter(!is.na(cat4facet))

(Figure_S4 <- Figure_S4_data %>%
  ggplot(aes(x=guard_duration, fill=ants)) +
  geom_histogram(binwidth = 4) +
  theme_publilia() +
  xlab("Guarding duration") +
  ...
  geom_vline(xintercept = 20, linetype="dashed", 
                color = "gray", size=1) +
  ggh4x::facet_grid2(vars(cat4facet, ants), 
                     strip = hist_strips) +
  theme(strip.background = element_rect(color = NA),
        strip.text = element_text(size = 12),
        legend.justification = c(1,1),
...
# 2023.07.22 - Using geom_ridgeline instead of freq_poly with alpha
Sarah asked to remove the stats text
...
Fig1_v1D +
  ...
  xlab("Clutch age (days) when first visited") +
  ylab("Guard duration of initiating female") +
  theme(legend.position = "none")
```

# 2023.07.22 - Upset plot to clarify sample sizes

## Upset plot
```{r, fig.width=6, fig.height=2}
sample_size_upset_data <- strats_data %>%
  group_by(location) %>%
    mutate(visited_1st_4d = any(!mom_at_init & as.numeric(visit_starts-initiation_date)<=4)) %>%
  ungroup() %>%
  mutate(`visited in 4 days` = ifelse(visited_1st_4d | num_at_init>1,

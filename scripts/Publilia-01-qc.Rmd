---
title: '01 - TreehopperFitness - Quality control and data prep'
...
# write_csv(egg_text_conflict_data, here("data/manual_check/egg_text_conflict_data.csv"))
...
# write_csv(egg_status_missing_data, here("data/manual_check/egg_status_missing_data.csv")
...
check_hatch_date <- All_data_v4  %>%
  ...
  distinct(location)
# write_csv(check_hatch_date, here("data/manual_check/check_hatch_date_tmp.csv")
```
...
# write_csv(ant_text_conflict, here("data/manual_check/ant_text_conflict.csv") # resolved 2019-10-08
```
...
# write_csv(short_unhatched_days_check, here("data/manual_check/short_unhatched_days_check_tmp.csv")

# *** when is this used for revisions, if ever?
never_hatched_check <- clutch_status_stats_v1 %>% 
  filter(is.na(hatch_date))

# write_csv(never_hatched_check, here("data/manual_check/never_hatched_check_tmp.csv")

# write_csv(clutch_status_stats_v1, here("data/manual_check/egg_measures_check_tmp.csv")
```
...
# write_csv(NOr_manual_check_table %>% filter(revise_NOr == "MANUAL CHECK"), here("data/manual_check/NOr_manual_check_tmp.csv")
...
```{r}
visited_1st_4d_data <- left_join(clutch_status_guarding_summary_v2, last_day_fID) %>%
  group_by(location) %>%
...
## 10.1 - Calculate median of top 5 egg counts in lieu of max egg counts
...
```{r, warning=FALSE}
t5eggs_data <- clutch_status_init_hatch_labels_v1 %>%
...
## 10.2 - Generate data for fecundity modeling
...
```{r}
strats_data <- visited_1st_4d_data_tb  %>%
  # Add ant arrival data
  left_join(ant_data) %>%
  ...
  mutate(vis_cat = ifelse(num_at_init > 1,
         ...
         vis_cat = factor(vis_cat, levels = c("0","1 to 4",
                                              "5+","never visited"))) %>%
  # Accommodate different tie-breaking procedures (wrt choosing a single female as initiator if multiple present at hatch)
  # 1) Longest initiator
  mutate(initL = init_first_dur_rank == 1) %>% 
  # 2) Shortest initiator
  #  i) break ties by picking marked females over unmarked
...
write_rds(strats_data, here("saved_rds/strats_data.rds"))
```
...
# 13 - Write data for `Publilia-04-within_clutch_iteroparity.Rmd` to `.rds`

## `Publilia-04-within_clutch_iteroparity.Rmd`
```{r}
write_rds(clutch_egg_num_data, here("saved_rds/clutch_egg_num_data.rds"))
write_rds(t5eggs_data, here("saved_rds/t5eggs_data"))
write_rds(fec_model_data, here("saved_rds/fec_model_data.rds"))
write_rds(clutch_status_init_hatch_labels_v1, here("saved_rds/clutch_status_init_hatch_labels_v1.rds"))
```

# 14 - Write data for 

```{r}
write_rds(moms_plant_data, here("saved_rds/moms_plant_data.rds"))
write_rds(focalMb4h_sc2h, here("saved_rds/focalMb4h_sc2h.rds"))
write_rds(clutch_survivorship_data, here("saved_rds/clutch_survivorship_data.rds"))
```

# 15 - Write pre-processed data for publishing with manuscript

```{r}
write_csv(All_data_v6, here("data/PubliliaFitness_raw_data.csv"))
```

---
title: "Publilia_2019_CHAPTER-0-prep"
author: "micah fletcher"
date: "1/18/2022"
output: html_document
---

# Publilia 2019 Chapter - Script 0: data prep
...
The outline for the chapter and the corresponding scripts are as follows:

0) *Data pre-processing:* 
Preprocessing necessary for all the following analyses, including various automated and manual checking procedures (Female IDs, numbers of eggs, nymphs, ants etc.).

1) *Guarding strategies:* Three distinct strategies for maternal care emerge from ant-dependent and secondary visitor-dependent plasticity
  - *Dataset A* requirements:
    - Guarding durations (relative to [estimated] hatching) are accurate.
      - The sample of clutches is representative of the full factorial of ants / no ants and visited early / not visited early
    - Able to compare 
      - initiated vs. visited
      - first vs. later [initiated] clutches
        -however, if plotting "all clutches" I can include unmarked females
      - disappeared vs. deserted (based on last day a female was observed)
    - Deciding on who is the "true" initiator is necessary for assumptions of independence for statistics. 
      - Ultimately, comparing the two most obvious options (first and longest guarding mom at initiation, vs first and shortest guarding mom at initiation) will need to suffice.
...
4) *Plant-level survivorship:* Do nymph migration and ant availability at a plant-level make plant-level dynamics more informative to survivorship than clutch-level?
  - Dataset F [survivorship ~ female presence + ant presence + number of clutches on plant]
most of the same metrics as for datasets D and E but with relaxed filtering of clutches with ambiguous nymph origins

5) *Ovary development:* If I have time (Narrator: He won't)

## Notes on previous versions.
Started quality checking in Fall 2019 in `female_ID_qc.R`. Started rewriting code in Rmd format on 3/11/2020 in `Watershed_2019_Analysis_IP.Rmd`. Started reformatting code into multiple scripts, separating data prep from analysis for better readability, reproducibility, and modularity on 4/13/2021. This final version was started 1/18/2022.
...
setwd("/Users/micah/Documents/Princeton/Kocher_Lab/Publilia/PubliliaBehavEcol/Watershed_2019/Watershed_2019_analysis/CHAPTER")
```
...
in_filename <- "../data_in_progress/Watershed_field_data_2019_ALL_local.xlsx"
...
#write_csv(Days_observed_dupStatus, "Days_observed_dupStatus_20200319.csv")
```

## 1.3 - Revise Female_ID using `Female_ID revisions dupStatus` sheet
- ~~check that each "From ID" is actually in the same location in the data as on the revisions table, flag where not the case~~ *[DONE]*
- ~~check that all duplicates have been resolved~~ *[DONE]*
- ~~manually doublecheck the location on date table (especially the entry for E_Rgr?_d1)~~ *[DONE]*
- *****manually check that `short_unhatched_days_check` female revisions remain appropriate and make these revisions reversible -> Check this after running the "short_unhatched_days_check" code chunk*****
-  ~~old method of replacing a NA (in `From ID`) with a Female_ID (in `To ID`) doesn't work; I will make a new `NA to ID` column that can add a Female ID where there was an NA before~~ *[DONE]*
```{r}
...
#write_csv(egg_status_missing_data_eggs_revised, "egg_status_missing_data_eggs_revised_20200326.csv")
```
...
nymph_status_to_resolve <- nymph_status  %>% # should only include a handful of entries with "?" ambiguous entries for N1 or N2 (because did the revions by hand on 2019/09/26)
  ...
  ungroup()
# write_csv(nymph_status_to_resolve, "nymph_status_to_resolve.csv") ## manually added this to "nymph_status revisions" sheet in "Watershed_field_data_2019_ALL_local" and manually revised raw data 2019/09/26
...
# 6 - Saving data for downstream analysis

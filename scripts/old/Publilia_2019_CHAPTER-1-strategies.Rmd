---
title: "Publilia_2019_CHAPTER-1-strategies"
...
(See `Publilia_2019_CHAPTER-0-prep.Rmd` for the full outline of the chapter)
...
# clutch_status_guarding_summary_v2 <- read_csv("../Publilia_2019_ABS_talk_analysis_data/clutch_status_guarding_summary_v2")
# last_day_fID <- read_csv("../Publilia_2019_ABS_talk_analysis_data/last_day_fID")
...
# 2 - Histograms and related stats
...
strat_data_table1 %>%
  as_flex_table() %>%
  flextable::save_as_docx(path="strat_data_table1.docx")
...
strat_data_table2 %>%
  as_flex_table() %>%
  flextable::save_as_docx(path="strat_data_table2.docx")
...
*Dealing with non-independence of multi-initiators: Using only longest or shortest initiator*
Sample sizes
```{r}
...
### How does including the females that were never observed again affect the results? 
...
## Figure 2
*Are early-visited females more likely to desert early?*
...
*Longest initiators only, disappeared females excluded*
...
#### Non-parametric version, but including females that disappeared or were unmarked
...
ggsave("guard_dur.vs.first_visit.v2.png",
       vis_start.vs.guard_dur.v2.stats.nonpara.p,
       device = "png",
       width = 160, height = 120, units = "mm", dpi = "print")
```

### Comparing early desertion by initiator vs. latest desertion of leaf by any female vs. latest desertion of the plant by any female
```{r}
...
ggsave("des_vs_desL_vs_desP.png",
...
ggsave("des_vs_desL_vs_desP.v2.png",
...
### Save selected figures for chapter
...
ggsave("desertion_firstclutch_hist.png",
       desertion_firstclutch_hist,
       device = "png",
       width = 120, height = 140, units = "mm", dpi = "print")

ggsave("guard_dur.vs.first_visit.png",
       guard_dur.vs.first_visit,
       device = "png",
       width = 160, height = 120, units = "mm", dpi = "print")

ggsave("desertion_firstclutch_hist_v2.png", # include females that disappeared
       Fig1_v2AB,
       device = "png",
       width = 120, height = 140, units = "mm", dpi = "print")
```
...
ggsave("duration_firstclutch_hist.png",
...
ggsave("FPO_freqpoly_all.png", FPO_freqpoly_all,
...
ggsave("FPO_freqpoly_noVis_all.png", FPO_freqpoly_noVis_all,
       device = "png", width = 120, height = 80, units = "mm", dpi = "print")
...
ggsave("FPO_freqpoly_noVis_black.png", FPO_freqpoly_noVis_black,
       device = "png", width = 120, height = 80, units = "mm", dpi = "print")
